...
<<<<<<< HEAD:Assets/BattleLogic.cs
        int successfulHit = Random.Range(1, 10);
        if (successfulHit > 3)
        {
...
=======
        ...
        if(CombatEnemyData.commonCombatEnemyParty[0].currentHealth == 0)
        {
            state = BattleState.WIN;
            EndBattle();
>>>>>>> 0e9603c74fd74bf708297ce118c8ffcd51b40132:Assets/Combat Scripts/BattleLogic.cs
        }
        else
        {
...
<<<<<<< HEAD:Assets/BattleLogic.cs
    ...
    IEnumerator PlayerMelee()
    {
        int successfulHit = Random.Range(1, 10);
        if (successfulHit > 5) {
            bool isDead = enemyInfo.TakeDamage(Random.Range(25, 50));
            enemyUI.SetHp(enemyInfo.currentHP);
            dialogue.text = "The attack is successful";
            yield return new WaitForSeconds(time);
            if (isDead)
    ...
    IEnumerator PlayerRanged()
    {
        ...
        if (successfulHit > 2)
        {
            bool isDead = enemyInfo.TakeDamage(Random.Range(5, 25));
            enemyUI.SetHp(enemyInfo.currentHP);
            dialogue.text = "The attack is successful";
            yield return new WaitForSeconds(time);
            if (isDead)
